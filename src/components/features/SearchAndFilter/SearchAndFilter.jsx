...
import { usePopup, ARIA_HASPOPUP } from "@lib/Overlay";
...
const SearchAndFilter = ({ onSearchChange, onFilterChange }) => {
    ...
    const popupControls = usePopup({
        bodyId: "filter-menu-overlay",
        triggerId: "filter-menu-trigger",
    ...
    const handleMenuChange = (event, { selectedKeys }) => {
        ...
        const newCategories = selectedArray.filter((key) =>
            selectedCategoryIds.includes(key),
        );
                    ...
                    {selectedPrices
                        ...
                        .map((priceId) => (
                            ...
                            >
                                Price:{" "}
                                {
                                    priceRanges.find((p) => p.id === priceId)
                                        ?.name
                                }
                                <button
                                    type="button"
                                    onClick={() => {
                                        const newPrices = selectedPrices.filter(
                                            (p) => p !== priceId,
                                        );
                                        setSelectedPrices(newPrices);
                                        onFilterChange({
